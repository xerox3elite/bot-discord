...
class ArsenalConfigUltimate(commands.Cog):
    ...
    config_group = app_commands.Group(name="config", description="🚀 Configuration Ultimate Arsenal")

    @config_group.command(name="show", description="🚀 Afficher la configuration Ultimate Arsenal")
...
class SystemConfigModal(discord.ui.Modal):
    ...
    @config_group.command(name="moderation", description="🛡️ Configurer la modération")
    async def config_moderation(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            ...
            value="• AutoMod Discord natif\n• IA de modération\n• Protection anti-raid\n• Quarantine système",
            inline=False
        )
        
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @config_group.command(name="tickets", description="🎫 Configurer les tickets")
    async def config_tickets(self, interaction: discord.Interaction):
        ...
        if not ticket_system:
            embed = discord.Embed(
                ...
                description="Le système de tickets n'est pas chargé.",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        embed.add_field(
            ...
            value="\n".join([f"{cat['emoji']} {cat['name']}" for cat in categories.values()]) or "Aucune catégorie",
            inline=False
        )
        
        embed.add_field(
            name="⚙️ Commandes disponibles",
            value="• `/ticket` - Créer un ticket\n• `/ticket setup` - Configurer\n• `/ticket panel` - Créer panneau",
            inline=False
        )
    ...
    @config_group.command(name="permissions", description="🔐 Configurer les permissions")
